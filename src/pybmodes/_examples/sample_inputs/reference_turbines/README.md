...
etc.) are **not** generated by `build.py` yet — those need
HydroDyn + WAMIT + MoorDyn integration that `build.py` doesn't drive
currently. The downstream `Tower.from_elastodyn_with_mooring(...)`
classmethod (added in 0.4.0; see CHANGELOG) handles the OpenFAST
side end-to-end for coupled-frequency prediction, but the
*pre-baked sample BMI* assembly equivalent for these floaters is
still on the roadmap. For the OC3 Hywind, the canonical
`OC3Hywind.bmi` (gitignored under `docs/BModes/docs/examples/`) is
also a valid input — the bundled sample at
[`07_nrel5mw_oc3hywind_spar/`](07_nrel5mw_oc3hywind_spar/) is a
re-emission with citations to Jonkman (2010) for redistributability.
