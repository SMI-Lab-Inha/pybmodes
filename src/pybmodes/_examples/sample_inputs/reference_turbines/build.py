...
The OC3 Hywind floating spar is generated here (sub-case 07) with
its full PlatformSupport block (hydro / mooring / inertia 6 × 6
matrices) populated inline from Jonkman 2010 NREL/TP-500-47535
Table 9-1; the build doesn't read an upstream deck for that sub-
case. The other floating reference turbines (IEA-15 UMaine semi,
IEA-22 semi) are NOT generated by this script — those need HydroDyn
+ WAMIT + MoorDyn integration that build.py doesn't drive. The
``Tower.from_elastodyn_with_mooring(...)`` classmethod handles the
end-to-end OpenFAST → coupled modal solve path for those decks
directly.
