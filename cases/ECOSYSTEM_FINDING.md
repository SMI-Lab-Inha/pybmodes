...
## Counterpoint: the *structural* blocks ARE consistent with the WindIO source

Everything above is about the *polynomial-coefficient* blocks drifting
away from the *structural-property* blocks in the same deck. A natural
worry is that the structural blocks themselves are also untrustworthy —
that the whole deck is internally inconsistent. They are not, and the
WindIO geometry path (added 1.4.0, issue #35) gives an independent,
machine-exact proof for at least one turbine.

`pybmodes.io.windio.read_windio_tubular` reads a WindIO ontology
`.yaml` — outer diameter, summed layer thicknesses, isotropic steel
`(E, ρ, ν)`, `outfitting_factor`, and the reference-axis span — and
`tubular_section_props` turns that geometry into distributed
`mass_den` / `EI` / `GJ` / `EA` by the exact closed-form circular-tube
identities. Comparing that *forward-from-geometry* result against the
*tabulated* distributed mass / EI columns in the same turbine's
OpenFAST ElastoDyn tower deck is a like-for-like cross-code check that
touches none of the polynomial machinery:

| Turbine | WindIO yaml → ElastoDyn tower table | max mass error | max FA-EI error |
| --- | --- | ---: | ---: |
| **IEA-15-240-RWT** | IEA Wind Task 37 base ontology → IEA-15 *Monopile* OpenFAST deck | **7.5 × 10⁻¹²** | **7.5 × 10⁻¹²** |
| IEA-3.4-130-RWT | IEA Task 37 ontology → own ElastoDyn tower deck | ~9 % | ~21 % |
| IEA-10.0-198-RWT | (same) | ~8 % | ~11 % |
| IEA-22-280-RWT | (same) | ~3 % | ~6 % |

The IEA-15 row is **machine-exact**: the IEA-15 Monopile ElastoDyn
tower table was itself generated from this WindIO geometry by the
WISDEM toolchain, and pyBmodes' independent closed-form tube reduction
reproduces it to the last bit. That is the cleanest possible evidence
that the *structural* blocks of an RWT deck faithfully encode the
published geometry — the silent inconsistency this report documents
lives **entirely in the polynomial-coefficient blocks**, not in the
distributed-property blocks they're supposed to be consistent with.

The IEA-3.4 / 10 / 22 rows are *not* machine-exact (3–21 %): their
shipped ElastoDyn tower tables were not regenerated by a 1:1 geometry
round-trip from the ontology yaml (a different revision-drift story
from the polynomial one — the structural table and the yaml are from
adjacent but non-identical model revisions). They still land within a
same-turbine envelope, confirming the geometry path produces
correctly-sized properties across both WindIO key dialects (modern
`outer_shape`/`structure` and the older `outer_shape_bem`/
`internal_structure_2d_fem`, plus IEA-10's duplicate-anchor file).

Practical consequence: when `pybmodes patch` regenerates the
polynomial blocks from the deck's structural inputs, those structural
inputs are themselves trustworthy — for IEA-15, provably the exact
published geometry. The fix is sound at both ends. This is gated by
[`tests/test_geometry_windio.py`](../tests/test_geometry_windio.py)
(the IEA-15 exact anchor, the older-dialect ballpark, and the
full-corpus parse/modal-smoke matrix) and tabulated in
