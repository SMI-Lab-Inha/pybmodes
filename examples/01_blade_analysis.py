...
Computes the natural frequencies and mode shapes for the CertTest01 non-uniform
rotating blade (35 m, 60 RPM), then fits 6th-order polynomials to the first
flap and edge modes ready for use in ElastoDyn.

Run from the repository root:
    conda run -n pybmodes python examples/01_blade_analysis.py
"""
...
def main() -> None:
    ...
    print("\nElastoDyn mode shape coefficients (BldFl1Sh / BldFl2Sh / BldEdgSh)")
    print(f"  {'Name':<12}  {'C2':>10}  {'C3':>10}  {'C4':>10}  {'C5':>10}  {'C6':>10}  {'RMS':>8}")
    print(f"  {'-'*12}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*10}  {'-'*8}")
    for name, fit in [("BldFl1Sh", params.BldFl1Sh),
        ...
        assert abs(fit.c2 + fit.c3 + fit.c4 + fit.c5 + fit.c6 - 1.0) < 1e-10, \
            "Constraint C2+…+C6=1 violated"

    # ── 4. Show first flap mode shape ─────────────────────────────────────────
    flap1 = result.shapes[0]
    print("\nFirst flap mode shape (span_loc vs flap_disp, normalised to tip=1):")
